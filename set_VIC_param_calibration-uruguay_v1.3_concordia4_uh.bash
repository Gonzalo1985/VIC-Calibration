...
path_out_vic='/home/gonzalo/Outputs/out-vic-4/' # modificar
path_out_route='/home/gonzalo/Outputs/out-rout-4/' # modificar
...
archivo_suelo_aux='soil_aux--Calibracion4.prn' 

# NOMBRES DE ARCHIVOS DE CONTROL DE LOS MODELOS
control_file_VIC='control_file_VIC-Calibracion4.prn'
control_file_route='control_file_route_calibracion_4.prn'
...
salida_RMSE='RMSE_uruguay_ByMyA--Calibracion_ERA_TERA4_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
salida_NS='N-S_uruguay_ByMyA--Calibracion_ERA_TERA4_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
#####################################################################
...
line="SOIL            ${path_control_vic}soil_aux--Calibracion4.prn"
...
line="${path_control_rout}Loca_7p4_gonza.asc"
...
for combinacion in {1..4}-{4..4}-{1..6}-{1..1}-{1..7}-{1..5}-{1..5}
do
rm ${path_ppal}parametros-cal4.R
rm ${path_ppal}estadisticos-cal4.R
...
cd ${path_ppal}
echo ${cal_param_1} ${cal_param_2} ${cal_param_3} ${cal_param_4} ${cal_param_5} ${cal_param_6} ${cal_param_7} >> ${path_ppal}LOG4.txt
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}parametros-cal4.R
################################################################################################

Rscript ${path_ppal}'parametros-cal4.R' >> ${path_ppal}LOG4.txt
...
mod.PELOT <- read.table(paste0(path.modelo, 'PELO4.day'), header = FALSE)
mod.CANOA <- read.table(paste0(path.modelo, 'CANO4.day'), header = FALSE)
mod.IRAI <- read.table(paste0(path.modelo, 'IRAI4.day'), header = FALSE)
mod.SOBER <- read.table(paste0(path.modelo, 'SOBE4.day'), header = FALSE)
mod.GARRU <- read.table(paste0(path.modelo, 'GARR4.day'), header = FALSE)
mod.PASOL <- read.table(paste0(path.modelo, 'PASO4.day'), header = FALSE)
mod.CONCO <- read.table(paste0(path.modelo, 'CONC4.day'), header = FALSE)
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}estadisticos-cal4.R

###################################################################################################

Rscript ${path_ppal}'estadisticos-cal4.R' >> ${path_ppal}LOG4.txt
