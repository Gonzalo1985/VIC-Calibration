...
path_out_vic='/home/gonzalo/Outputs/out-vic-6/' # modificar
path_out_route='/home/gonzalo/Outputs/out-rout-6/' # modificar
...
archivo_suelo_aux='soil_aux--Calibracion6.prn' 

# NOMBRES DE ARCHIVOS DE CONTROL DE LOS MODELOS
control_file_VIC='control_file_VIC-Calibracion6.prn'
control_file_route='control_file_route_calibracion_6.prn'
...
salida_RMSE='RMSE_uruguay_ByMyA--Calibracion_ERA_TERA6_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
salida_NS='N-S_uruguay_ByMyA--Calibracion_ERA_TERA6_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
#####################################################################
...
line="SOIL            ${path_control_vic}soil_aux--Calibracion6.prn"
...
line="${path_control_rout}Loca_7p6_gonza.asc"
...
for combinacion in {1..4}-{6..6}-{1..6}-{1..1}-{1..7}-{1..5}-{1..5}
do
rm ${path_ppal}parametros-cal6.R
rm ${path_ppal}estadisticos-cal6.R
...
cd ${path_ppal}
echo ${cal_param_1} ${cal_param_2} ${cal_param_3} ${cal_param_4} ${cal_param_5} ${cal_param_6} ${cal_param_7} >> ${path_ppal}LOG6.txt
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}parametros-cal6.R
################################################################################################

Rscript ${path_ppal}'parametros-cal6.R' >> ${path_ppal}LOG6.txt
...
${path_route}rout ${control_file_route} >> ${path_ppal}LOG6.txt
...
mod.PELOT <- read.table(paste0(path.modelo, 'PELO6.day'), header = FALSE)
mod.CANOA <- read.table(paste0(path.modelo, 'CANO6.day'), header = FALSE)
mod.IRAI <- read.table(paste0(path.modelo, 'IRAI6.day'), header = FALSE)
mod.SOBER <- read.table(paste0(path.modelo, 'SOBE6.day'), header = FALSE)
mod.GARRU <- read.table(paste0(path.modelo, 'GARR6.day'), header = FALSE)
mod.PASOL <- read.table(paste0(path.modelo, 'PASO6.day'), header = FALSE)
mod.CONCO <- read.table(paste0(path.modelo, 'CONC6.day'), header = FALSE)
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}estadisticos-cal6.R

###################################################################################################

Rscript ${path_ppal}'estadisticos-cal6.R' >> ${path_ppal}LOG6.txt
