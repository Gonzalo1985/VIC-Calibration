...
path_out_vic='/home/gonzalo/Outputs/out-vic-1/' # modificar
path_out_route='/home/gonzalo/Outputs/out-rout-1/' # modificar
...
archivo_suelo_aux='soil_aux--Calibracion1.prn' 

# NOMBRES DE ARCHIVOS DE CONTROL DE LOS MODELOS
control_file_VIC='control_file_VIC-Calibracion1.prn'
control_file_route='control_file_route_calibracion_1.prn'
...
salida_RMSE='RMSE_uruguay_ByMyA--Calibracion_ERA_TERA1_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
salida_NS='N-S_uruguay_ByMyA--Calibracion_ERA_TERA1_2004-2010_Cascade_PELOTASyCANOAS.prn' # modificar
#####################################################################
...
line="SOIL            ${path_control_vic}soil_aux--Calibracion1.prn"
...
line="${path_control_rout}Loca_7p1_gonza.asc"
...
for combinacion in {1..4}-{1..1}-{1..6}-{1..1}-{1..7}-{1..5}-{1..5}
do
rm ${path_ppal}parametros-cal.R
rm ${path_ppal}estadisticos-cal.R
...
cd ${path_ppal}
echo ${cal_param_1} ${cal_param_2} ${cal_param_3} ${cal_param_4} ${cal_param_5} ${cal_param_6} ${cal_param_7} >> ${path_ppal}LOG1.txt
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}parametros-cal.R
################################################################################################

Rscript ${path_ppal}'parametros-cal.R' >> ${path_ppal}LOG1.txt
...
${path_VIC}vicNl -g ${path_control_vic}${control_file_VIC}
rm ${path_files}${archivo_suelo_aux}

cd ${path_control_rout}
rm *1.uh_s

${path_route}rout ${control_file_route} >> ${path_ppal}LOG1.txt
...
mod.PELOT <- read.table(paste0(path.modelo, 'PELO1.day'), header = FALSE)
mod.CANOA <- read.table(paste0(path.modelo, 'CANO1.day'), header = FALSE)
mod.IRAI <- read.table(paste0(path.modelo, 'IRAI1.day'), header = FALSE)
mod.SOBER <- read.table(paste0(path.modelo, 'SOBE1.day'), header = FALSE)
mod.GARRU <- read.table(paste0(path.modelo, 'GARR1.day'), header = FALSE)
mod.PASOL <- read.table(paste0(path.modelo, 'PASO1.day'), header = FALSE)
mod.CONCO <- read.table(paste0(path.modelo, 'CONC1.day'), header = FALSE)
...
###########################################################################
############################ FINALIZADO ###################################
###########################################################################" >> ${path_ppal}estadisticos-cal.R

###################################################################################################

Rscript ${path_ppal}'estadisticos-cal.R' >> ${path_ppal}LOG1.txt
